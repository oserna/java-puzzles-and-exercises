...
/**
 * The datatype.tree is traversed in depth first
 * 
 * @author oserna
 *
 * @param <T>
...
public class DepthFirstSearch<T> {
	...
	public void dfs(Node<T> root){
		...
		while (!stack.isEmpty()) {
			...
			//give the first child that hasn't been visited, if is a binary datatype.tree usuallu left, and right later
	...
	public void dfs2(Node<T> root){
		...
		while (!stack.isEmpty()) {
			...
			if (!pop.isVisited()) {
				...
				//check for the finish condition if exists. If we want to traverse the whole datatype.tree it's not necessary
